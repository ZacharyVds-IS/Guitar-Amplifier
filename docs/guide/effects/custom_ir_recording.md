...
2. Setup a microhpone and a recording device (e.g. a laptop and a microphone).
...
This DIY method isn't ideal since the clap is mainly in the mid tones. Idealy we want to know the reaction for all frequencies.

On a professional level for recording an amp's impulse response for ex.
1. is to place a microhpone in front of the speaker cabinet (like required for normal recording).
