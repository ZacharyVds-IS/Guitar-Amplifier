...
impl AudioProcessor for ToneStackProcessor {
    fn process(&mut self, sample: f32) -> f32 {
        self.bass_eq.set_percent(self.tone_stack.bass().load(Ordering::Relaxed));
        self.mid_eq.set_percent(self.tone_stack.middle().load(Ordering::Relaxed));
        self.treble_eq.set_percent(self.tone_stack.treble().load(Ordering::Relaxed));

        let mut processed = self.bass_eq.process(sample);
        processed = self.mid_eq.process(processed);
        processed = self.treble_eq.process(processed);
        processed
    }
}