/**
 * Auto-generated TypeScript bindings for Tauri commands
 * Generated by tauri-typegen v0.5.0
 * Generated at: 2026-04-28T13:37:13.219280500+00:00
 * Generator: none
 *
 * Do not edit manually - regenerate using: cargo tauri-typegen generate
 */

/**
 * Event Listeners
 * Type-safe event listener helpers for Tauri events
 */
import {listen, type UnlistenFn} from '@tauri-apps/api/event';

/**
 * Listen for 'channel-added' events
 * @param handler - Callback function to handle the event
 * @returns Promise that resolves to an unlisten function
 */
export async function onChannelAdded(
  handler: (payload: unknown) => void
): Promise<UnlistenFn> {
  return listen<unknown>('channel-added', (event) => {
    handler(event.payload);
  });
}


/**
 * Listen for 'channel-created' events
 * @param handler - Callback function to handle the event
 * @returns Promise that resolves to an unlisten function
 */
export async function onChannelCreated(
  handler: (payload: string) => void
): Promise<UnlistenFn> {
  return listen<string>('channel-created', (event) => {
    handler(event.payload);
  });
}


